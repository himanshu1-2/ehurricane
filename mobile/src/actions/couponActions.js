...
export const listCoupons = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(COUPONS_URL, authConfig(userInfo));
...
export const listCouponDetails = (id) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(`${COUPONS_URL}/${id}`, authConfig(userInfo));
...
export const deleteCoupon = (id) => async (dispatch, getState) => {
  try {
    ...
    await axios.delete(`${COUPONS_URL}/${id}`, authConfig(userInfo));
...
export const createCoupon = (coupon) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.post(COUPONS_URL, coupon, authConfig(userInfo, true));
